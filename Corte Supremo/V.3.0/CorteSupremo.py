...
def corteSupremo ():
    ...
    #PROBLEMA
    f1 = [0.93, 0.03, 0.0, 0.04]
    f2 = [0.51, 0.46, 0.01, 0.02]
    torta = [0.78, 0.01, 0.04, 0.17] 
    ...
    puntosMediosDeF2 = []
    porcentaje = 0
    precision = 0.01 # precision del 1%
    gananciaMaxF1 = -1.0
    
    while (porcentaje < 1):
        
        # Revisar que la porcion de mayor ganancia para cada usuario sea de un lado diferente (u1: izq, u2:der) y
        #   si eso se cumple guardar en una varible y comparar si es que no hay otro valor con mayor ganancia para
        #   u1 que cumpla con lo anterior
        
        # Sino, y si ambas ganancias del usuario 1 son iguales y la ganancia es mayor o igual, cortar en esa parte
        #   y tomar el lado en el que el usuario 2 tiene la menor ganancia
        ...
        if (ganancia(parteIzquierda, f2) == ganancia(parteDerecha, f2)):
            
            puntosMediosDeF2.append(porcentaje) 
        
        porcentaje += precision
    
    #Si no encuentro un punto claro donde cortar la torta
    if (mejorDivision["Usuario1"] == []): 
        
        #Comienzo a analizar los puntos donde si corto la torta f2 es lo mismo para ambas partes
        for puntoMedio in puntosMediosDeF2:
            
            parteIzquierda, parteDerecha = dividirEnPartes (puntoMedio, torta)
            
            gananciaF1ParteIzquierda = ganancia(parteIzquierda, f1)
            gananciaF1ParteDerecha = ganancia(parteDerecha, f1)
            
            #Si en alguno de esos puntos f1 es igual en ambos lados corto en ese punto
            if gananciaF1ParteIzquierda == gananciaF1ParteDerecha:
                
                mejorDivision["Usuario1"] = parteIzquierda
                mejorDivision["Usuario2"] = parteDerecha
                
                gananciaMaxF1 = gananciaF1ParteIzquierda    
                
            else:
                
                #Si es mejor el lado izquierdo para f1 corto un valor de precision hacia la izquierda favoreciendo el derecho para f2
                if gananciaF1ParteIzquierda > gananciaMaxF1:
                    
                    parteIzquierda, parteDerecha = dividirEnPartes (puntoMedio - precision, torta)
                ...
                #Si es mejor el lado izquierdo para f1 corto un valor de precision hacia la derecha favoreciendo el Izquierdo para f2
                if gananciaF1ParteDerecha > gananciaMaxF1:
                    ...
                    gananciaMaxF1 = gananciaF1ParteDerecha
